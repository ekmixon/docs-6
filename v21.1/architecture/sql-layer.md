...
When a node in a CockroachDB cluster receives a SQL request from a client, it [parses the statement](#parsing), [creates a query plan](#logical-planning), and then [executes the plan](#query-execution).
